...
MOCK_MILK_SEARCH_PAGE = '/home/daycu/Programming/Meaningfy/sem-covid/tests/test_data/crawlers/sample_ireland_gov/milk_keyword.html'
MOCK_INNOVATION_SEARCH_PAGE = '/home/daycu/Programming/Meaningfy/sem-covid/tests/test_data/crawlers/sample_ireland_gov/mock_covid_search_terms_pages/innovation.html'
...
mock_milk_search_page_file = pathlib.Path(__file__).parent.parent.parent / 'tests' / 'test_data' / 'crawlers' / 'sample_ireland_gov' / 'saved_data' / 'mock_milk_search_page.json'
...
def fake_response_from_file(file_name: str, url: str = None) -> HtmlResponse:
    ...
    """
    if not url:
        url = 'https://www.gov.ie/en/publications/?q=milk&sort_by=published_date'
    if not file_name[0] == '/':
...
def test_irish_gov_crawler_parsing_detail_page_method():
    ...
    result = irish_gov_crawler.parse_detail_page(fake_response_from_file(MOCK_ARTICLE_PAGE))
    result = irish_gov_crawler.data
    mock_article_page_file.write_text(data=json.dumps(result))


def test_irish_gov_crawler_search_milk():
    irish_gov_crawler = IrishGovCrawler(
