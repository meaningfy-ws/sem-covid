...
def get_single_item(query, json_file_name, wrapperSPARQL, minio, transformer):
    ...
    legal_initiatives_json_dataset = transformer(response)
    uploaded_bytes = minio.put_object(json_file_name,
                                      dumps(legal_initiatives_json_dataset).encode('utf-8'))
    ...
    return legal_initiatives_json_dataset
...
def download_and_split_callable():
    ...
    transformed_json = get_single_item(QUERY, config.LEGAL_INITIATIVES_JSON, wrapperSPARQL, minio,
                                       transformer=json_transformer.transform_legal_initiatives)
