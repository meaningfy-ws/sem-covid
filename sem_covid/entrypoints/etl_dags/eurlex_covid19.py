...
def upload_processed_documents_to_elasticsearch():
    ...
    logger.info(f'Using ElasticSearch at {ELASTICSEARCH_PROTOCOL}://{config.ELASTICSEARCH_HOST}:{config.ELASTICSEARCH_PORT}')
...
get_config.EURLEX_TIMELINE_JSON_task = PythonOperator(
    task_id=f'Download',
    python_callable=get_covid19_items, retries=1, dag=dag)

download_documents_and_enrich_config.EURLEX_TIMELINE_JSON_task = PythonOperator(
    task_id=f'Enrich',
...
get_config.EURLEX_TIMELINE_JSON_task >> download_documents_and_enrich_config.EURLEX_TIMELINE_JSON_task >> extract_content_with_tika_task >> upload_to_elastic_task
